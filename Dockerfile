...
RUN [ -e /usr/bin/upx ] && upx /go/bin/nostr-hotpostrank || echo
FROM scratch
COPY --link --from=build-dev /go/bin/nostr-hotpostrank /go/bin/nostr-hotpostrank
COPY --from=build-dev /etc/ssl/certs/ca-certificates.crt /etc/ssl/certs/
CMD ["/go/bin/nostr-hotpostrank"]
